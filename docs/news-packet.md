...
台帳の更新(`scripts/import_work_news.py commit-pending`)は、以下が
すべて成功した後にのみ行う。
...
1. `scripts/import_work_news.py fetch` でWork Issueを確認する
2. 有効な新規Work記事がある → Work記事だけを使用する(exit 0)
3. 新規Work記事がない、gh未認証・通信失敗、またはIssueのJSON不正で
   有効な記事が0件 → 警告をstderrへ出し、既存RSS収集
   (`scripts/collect.py`)を使用する(exit 1、run.sh側でフォールバック
   分岐)
4. 同一実行内でWork記事とRSS記事を混在させない(run.shのif/elseで
   排他的に分岐する)

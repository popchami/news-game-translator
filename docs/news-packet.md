...
## 法律案・制度に関する記事のconfirmedFacts記載方針

法律案・制度改正を扱う記事では、`confirmedFacts` に以下を可能な限り
個別の事実として記録する(`summary` へ要約するだけに集約しない)。

- 対象範囲: 誰・何に適用されるか
- 法律・議案要旨にある正式な定義語: そのまま記録する
  (例: 親王・親王妃・内親王・王・王妃・女王・皇嗣・皇嗣妃)
- 適用条件: 具体的な要件
- 例外・対象外となるケース
- 現在の正式な手続段階: 提出/審議入り/委員会可決/本会議可決/成立/施行等

正式名称・専門用語は `summary` だけに集約せず、`confirmedFacts` にも
そのまま残す。これにより `scripts/validate.py` が「入力に存在する語」
として正しく認識でき、下流の `claude -p` 変換が正式用語を省略・言い換え
せず正確に扱いやすくなる(詳細は `config/runtime_rules.md` の
「基本原則(優先順位)」を参照)。
